                ...
                switch (choice) {
                    ...
                    case 3:
                        ...
                        if (chonSua == 1) {
                            ...
                            Document docSua = docManager.layDuLieu(maSua);

                            if (docSua == null) {
                                System.out.println("Không tìm thấy tài liệu với mã: " + maSua);
                            } else {
                                System.out.print("Nhập tên tài liệu mới: ");
                                String tenSua = sc.nextLine();
                                docSua.setTenTaiLieu(tenSua);
                                docManager.sua(maSua, docSua);
                                System.out.println("Sửa tên tài liệu thành công!");
                            }
                        } else if (chonSua == 2) {
                            System.out.print("Nhập mã tài liệu chứa bản sao: ");
                            String maDocSuaBS = sc.nextLine();
                            Document docSuaBS = docManager.layDuLieu(maDocSuaBS);

                            if (docSuaBS != null) {
                                System.out.print("Nhập mã bản sao cần cập nhật: ");
                                String maBSSua = sc.nextLine();
                                Copy copyCanSua = docSuaBS.getListBanSao().get(maBSSua.toLowerCase());

                                if (copyCanSua != null) {
                                    System.out.print("Chọn trạng thái mới (1. Còn tốt, 2. Hư hỏng, 3. Mất): ");
                                    int chonTTMoi = sc.nextInt();
                                    sc.nextLine();

                                    TrangThai ttMoi = TrangThai.CON_TOT;
                                    switch (chonTTMoi) {
                                        case 1:
                                            ttMoi = TrangThai.CON_TOT;
                                            break;
                                        case 2:
                                            ttMoi = TrangThai.HU_HONG;
                                            break;
                                        case 3:
                                            ttMoi = TrangThai.MAT;
                                            break;
                                    ...
                                    copyCanSua.setTrangThai(ttMoi);
                                    docSuaBS.suaBanSao(copyCanSua, copyCanSua);
                                    System.out.println("Cập nhật trạng thái bản sao thành công");
                                } else {
                                    System.out.println("Không tìm thấy bản sao mã: " + maBSSua);
                                }
                            } else {
                                System.out.println("Không tìm thấy tài liệu mã: " + maDocSuaBS);
                            }
                        } else {
