...
public class Lab3_Bai4 {
    public static void main(String[] args) {
        ...
        sv.stream().filter(s -> s.getType() == StudentType.INTERNATIONAL && s.getGpa() >= 3.2).forEach(s -> System.out.println(s));

        sv.stream().sorted((s1, s2) -> Double.compare(s2.getGpa(), s1.getGpa())).limit(3).forEach(s -> System.out.println(s));

        sv.stream().filter(s -> s.getType() == StudentType.PART_TIME).map(s -> s.getName()).forEach(s -> System.out.println(s));
    }
}
